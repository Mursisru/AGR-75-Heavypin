...
namespace Heavypin.Runtime
{
    ...
    internal static class HeavypinLauncherRockets
    {
        internal static void SetupMount(GameObject mountGo, Transform launcher, int want)
        {
            ...
            for (int i = 0; i < embedded.Count; i++)
            {
                ...
                HeavypinAnim.Park(rocket);
            }

            LinkSlots(mountGo, embedded);
            HeavypinPlugin.ModLog?.LogInfo(
                $"HeavypinLauncherRockets setup embedded={embedded.Count} want={want} launcher='{launcher.name}'");
        }
        ...
        internal static void LinkSlots(GameObject mountGo, List<Transform> embedded)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (slot == null)
                    continue;
                HeavypinSlotLink link = slot.GetComponent<HeavypinSlotLink>() ?? slot.gameObject.AddComponent<HeavypinSlotLink>();
        ...
        internal static void EnsureSlotLinks(GameObject mountGo)
        {
            ...
            {
                HeavypinSlotLink? link = slots[i]?.GetComponent<HeavypinSlotLink>();
        ...
        internal static void HideEmbedded(MountedMissile? mount)
        {
            ...
            if (rocket == null)
            {
                HeavypinPlugin.ModLog?.LogWarning($"Heavypin: HideEmbedded miss slot='{mount?.name}'");
                return;
            }
        ...
        internal static Transform? ResolveEmbedded(MountedMissile? mount)
        {
            if (mount == null)
                return null;
            HeavypinSlotLink? link = mount.GetComponent<HeavypinSlotLink>();
        ...
        internal static List<Transform> FindEmbedded(Transform launcher, int want)
        {
            ...
            for (int i = 0; i < all.Length; i++)
            {
                ...
                if (t == null || t == launcher || !IsEmbeddedRocketName(t.name))
                    continue;
                if (t.name == HeavypinConstants.RocketVisualName)
                    continue;
                fallback.Add(t);
            }
        ...
        internal static bool IsEmbeddedRocketName(string? name)
        {
            if (name is not { Length: > 0 } n)
                return false;
            const string root = HeavypinConstants.LauncherEmbeddedRocketName;
