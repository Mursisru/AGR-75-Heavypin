...
{
    internal static class HeavypinMotorFx
    {
        ...
        internal static void Capture(Encyclopedia enc, MissileDefinition? donor)
        {
            ...
            {
                HeavypinPlugin.ModLog?.LogWarning("HeavypinMotorFx: no AGR-24 for FX capture.");
                return;
            }
            ...
            if (_hold == null)
            {
                _hold = new GameObject("Heavypin_Agr24FxHold");
                UnityEngine.Object.DontDestroyOnLoad(_hold);
                _hold.SetActive(false);
            }

            CaptureArray(ParticlesField.GetValue(booster) as Array, PsTemplates, "HeavypinDonorPs", typeof(ParticleSystem));
            CaptureArray(TrailsField?.GetValue(booster) as Array, TrailTemplates, "HeavypinDonorTrail", typeof(TrailEmitter));
            CaptureAudio(AudioField?.GetValue(booster) as Array);
            CaptureArray(LightsField?.GetValue(booster) as Array, LightTemplates, "HeavypinDonorLit", typeof(Light));
            HeavypinPlugin.ModLog?.LogInfo(
                $"HeavypinMotorFx capture from '{def.jsonKey}' ps={PsTemplates.Count} trails={TrailTemplates.Count} audio={AudioTemplates.Count} lights={LightTemplates.Count}");
        }
        ...
        internal static void Bind(Missile missile)
        {
            if (missile == null || MotorsField == null || MotorType == null)
                return;
            if (HasChildNamed(missile.transform, "HeavypinExhaust"))
                return;
            ...
            if (nozzles.Count == 0)
            {
                HeavypinPlugin.ModLog?.LogWarning("Heavypin: EngineEffectsSpawn dummies missing.");
                return;
            }
        ...
        private static void InjectOnNozzles(Missile missile, object motor, List<Transform> nozzles)
        {
            ...
            for (int n = 0; n < nozzles.Count; n++)
            {
                ...
                {
                    GameObject? go = PlaceOnDummy(PsTemplates[i], sock, "HeavypinExhaust");
                    if (go == null)
                        continue;
                ...
                {
                    GameObject? go = PlaceOnDummy(TrailTemplates[i], sock, "HeavypinTrail");
                    if (go == null)
                        continue;
                ...
                {
                    GameObject? go = PlaceOnDummy(LightTemplates[i], sock, "HeavypinLight");
                    if (go == null)
                        continue;
            ...
            {
                GameObject? go = PlaceOnDummy(AudioTemplates[0], nozzles[0], "HeavypinAudio");
                if (go != null)
                {
            ...
            {
                GameObject go = new GameObject("HeavypinLight");
