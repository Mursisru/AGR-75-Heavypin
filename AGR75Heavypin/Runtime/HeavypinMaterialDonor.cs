...
{
    // Launcher embedded Rocket = visual truth. Fly HeavypinRocket clones these URP slots by mat name.
    internal static class HeavypinMaterialDonor
    {
        ...
        internal static void Ensure()
        {
            ...
            if (launcher == null)
            {
                HeavypinPlugin.ModLog?.LogWarning("HeavypinMaterialDonor: no launcher prefab.");
                return;
            }
            ...
            try
            {
                ...
                List<Transform> embedded = HeavypinLauncherRockets.FindEmbedded(launcherRoot, 1);
                if (embedded.Count == 0 || embedded[0] == null)
                {
                    HeavypinPlugin.ModLog?.LogWarning("HeavypinMaterialDonor: embedded Rocket missing.");
                    return;
                }
                ...
                HeavypinPlugin.ModLog?.LogInfo($"HeavypinMaterialDonor cached slots={Templates.Count}");
            }
            catch (Exception ex)
            {
                HeavypinPlugin.ModLog?.LogWarning($"HeavypinMaterialDonor: {ex.Message}");
            }
            finally
            {
                if (probe != null)
