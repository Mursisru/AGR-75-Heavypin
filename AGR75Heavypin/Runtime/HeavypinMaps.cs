...
{
    internal static class HeavypinMaps
    {
        ...
        private static Texture2D? Load(string fold, Dictionary<string, string> table, bool linear, string suffix)
        {
            ...
            try
            {
                ...
                tex.anisoLevel = 4;
                Cache[cacheKey] = tex;
                HeavypinPlugin.ModLog?.LogInfo($"HeavypinMaps loaded '{Path.GetFileName(file)}' {tex.width}x{tex.height}");
                return tex;
            }
            catch (Exception ex)
            {
                HeavypinPlugin.ModLog?.LogWarning($"HeavypinMaps '{file}': {ex.Message}");
                return null;
            }
        }
        ...
        private static void Scan()
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(Paths.PluginPath))
                    dirs.Add(Path.Combine(Paths.PluginPath, "AGR-75-Heavypin", "Textures", "AGR75"));
            }
            catch
            {
                // ignore
