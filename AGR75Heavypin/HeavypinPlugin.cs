...
namespace Heavypin
{
    ...
    public sealed class HeavypinPlugin : BaseUnityPlugin
    {
        internal static HeavypinPlugin? Instance { get; private set; }
        ...
        private void Awake()
        {
            ...
            try
            {
                ...
                ModLog.LogInfo($"AGR-75 Heavypin {AppVersion.Version} loaded (requires Blueprinter).");
            }
            catch (Exception ex)
            {
        ...
        internal void StartBootstrap(Encyclopedia enc)
        {
            if (enc == null)
                return;
            StartCoroutine(HeavypinBootstrap.Run(enc));
        }
    }
}
