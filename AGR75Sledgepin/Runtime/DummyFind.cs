...
namespace Sledgepin.Runtime
{
    internal static class DummyFind
    {
        ...
        internal static Transform? FindRocketCenter(Transform root) =>
            FindByAliases(root, SledgepinConstants.RocketCenterAliases);

        internal static Transform? FindPylonAttach(Transform root) =>
            FindByAliases(root, SledgepinConstants.AttachPylonAliases);
        ...
        internal static bool IsRocketSlotName(string? name)
        {
            if (string.IsNullOrEmpty(name))
                return false;
            if (string.Equals(name, SledgepinConstants.DummyPylonAttach, StringComparison.OrdinalIgnoreCase))
                return false;
            return StartsWith(name!, SledgepinConstants.DummySlot4Prefix) ||
                   StartsWith(name!, SledgepinConstants.DummySlot6Prefix);
        }

        internal static bool IsNozzleName(string? name)
        {
            return !string.IsNullOrEmpty(name) && StartsWith(name!, SledgepinConstants.DummyNozzlePrefix);
        }
