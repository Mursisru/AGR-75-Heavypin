...
namespace Sledgepin.Runtime
{
    internal static class VisualMaterials
    {
        ...
        internal static void ApplyFbxLook(GameObject root, bool flyRocket = false)
        {
            ...
            for (int i = 0; i < rs.Length; i++)
            {
                ...
                for (int m = 0; m < slots; m++)
                {
                    ...
                    if (flyRocket && SledgepinMaterialDonor.TryClone(matName, out Material? donated) && donated != null)
                    {
                        dst[m] = donated;
                        n++;
                        continue;
                    ...
                    Material? bakeRef = flyRocket
                        ? SledgepinMaterialDonor.GetBaked(matName) ?? old
                        : old;
                    bool isGlass = SledgepinMat004.IsName(bakeRef?.name ?? matName) ||
                                   SledgepinMat004.IsMainGlassSlot(r.gameObject.name, m, slots);

                    if (isGlass)
                    {
                        Material mat = VisualShader.MakeGlass(matName + "_hp");
                        Texture? albedo = SledgepinMaps.Albedo(matName) ?? PeekAlbedo(bakeRef);
                        if (albedo == null)
                            albedo = SledgepinMaps.Albedo("Материал.004");
                        SledgepinMat004.ApplyGlass(mat, albedo);
                        ApplyNormal(mat, matName, bakeRef);
                        KillEmission(mat);
                        SledgepinLook.ApplyFromBaked(mat, bakeRef, albedo != null);
                        dst[m] = mat;
                        n++;
                        continue;
                    }
                    ...
                    Texture? alb = SledgepinMaps.Albedo(matName) ?? PeekAlbedo(bakeRef);
                    ...
                    KillEmission(opaque);
                    SledgepinLook.ApplyFromBaked(opaque, bakeRef, albedoOwns);
                    dst[m] = opaque;
                    n++;
                }
            ...
            SledgepinPlugin.ModLog?.LogInfo($"VisualMaterials baked '{root.name}' fly={flyRocket} slots={n}");
        }
        ...
        {
            Texture2D? nml = SledgepinMaps.Normal(matName);
            if (nml != null)
            {
                Texture? bump = nml;
