...
namespace Sledgepin.Bootstrap
{
    internal static class PrefabFactory
    {
        ...
        internal static void ActivateMountedInstance(GameObject instance)
        {
            ...
            SledgepinLauncherRockets.EnsureSlotLinks(instance);
        ...
        internal static Transform? FindRocketVisual(Transform root) =>
            FindVisual(root, SledgepinConstants.RocketVisualName);

        internal static Transform? FindLauncherVisual(Transform root) =>
            FindVisual(root, SledgepinConstants.LauncherVisualName);
        ...
        internal static bool IsOurVisualRoot(Transform t)
        {
            ...
            {
                string n = t.name;
                if (n == SledgepinConstants.RocketVisualName || n == SledgepinConstants.LauncherVisualName)
                    return true;
                t = t.parent;
            }
            return false;
        ...
        private static void EnsureVisualRenderers(GameObject root)
        {
            ...
            SledgepinLauncherRockets.EnableEmbedded(root);
            SledgepinLauncherRockets.ParkEmbedded(root);
        }
        ...
        internal static bool IsOurMountKey(string? jsonKey) =>
            ...
            jsonKey!.StartsWith(SledgepinConstants.MountKeyPrefix, StringComparison.Ordinal);
        ...
        internal static void AssertDonorsIntact(Encyclopedia enc)
        {
            ...
            if (enc.missiles != null)
            {
                foreach (MissileDefinition m in enc.missiles)
                {
                    ...
                    if (!string.IsNullOrEmpty(m.unitName) &&
                        (m.unitName.IndexOf("Sledgepin", StringComparison.OrdinalIgnoreCase) >= 0 ||
                         m.unitName.IndexOf("Sledgepin", StringComparison.OrdinalIgnoreCase) >= 0))
                        SledgepinPlugin.ModLog?.LogError($"AGR-24 unitName mutated: '{m.unitName}' key='{m.jsonKey}'");
                }
            }
            if (enc.weaponMounts == null)
                return;
            foreach (WeaponMount m in enc.weaponMounts)
            {
                ...
                if (!AgrDonor.IsAgrMount(m))
                    continue;
                if (m.mountName != null &&
                    (m.mountName.IndexOf("Sledgepin", StringComparison.OrdinalIgnoreCase) >= 0 ||
                     m.mountName.IndexOf("Sledgepin", StringComparison.OrdinalIgnoreCase) >= 0))
                    SledgepinPlugin.ModLog?.LogError($"Donor corrupted: mountName '{m.mountName}' on '{m.jsonKey}'");
                if (m.info != null &&
                    !string.IsNullOrEmpty(m.info.weaponName) &&
                    (m.info.weaponName.IndexOf("Sledgepin", StringComparison.OrdinalIgnoreCase) >= 0 ||
                     m.info.weaponName.IndexOf("Sledgepin", StringComparison.OrdinalIgnoreCase) >= 0))
                    SledgepinPlugin.ModLog?.LogError($"Donor WeaponInfo mutated: '{m.info.weaponName}' on '{m.jsonKey}'");
            }
        }
    }
}
