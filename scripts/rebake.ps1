# Sync Blender FBX/textures → UnityBake → build AGR75Sledgepin.nobp → deploy
param(
    [string]$BlenderRoot = "C:\Users\at747_loyuw9y\OneDrive\Документы\Blender\AGR-75-Sledgepin",
...
Copy-Item -Force (Join-Path $BlenderRoot "AGR-75-Sledgepin-MainRocket.fbx") $missile
...
Copy-Item -Force (Join-Path $BlenderRoot "AGR-Textures\AGR-Rocket-Textures\AGR-Preview.png") (Join-Path $RepoRoot "AGR75Sledgepin\Resources\AGR-Preview.png")

$proj = Join-Path $RepoRoot "UnityBake"
$log = Join-Path $proj "batchbuild-gui.log"
$env:SLEDGEPIN_UNITY_EXIT = "1"
...
dotnet build (Join-Path $RepoRoot "AGR75Sledgepin\AGR75Sledgepin.csproj") -c Release
